...
const EditPayment = () => {
  ...
  const [coveredByInsuranceCompany, setCoveredByInsuranceCompany] = useState('');
    ...
    try {
        axios.get(`/payment-record/edit/${id}`,{
            ...
            if(res.data.status == 'success'){
                ...
                setCoveredByInsuranceCompany(res.data.payment.full_covered_by_insurance_company);
                setNumber(res.data.payment.number);
                setCost(res.data.payment.cost);
                setAdditionalPayment(res.data.payment.additional_payment ? res.data.payment.additional_payment : '');
                setHomeVisit(res.data.payment.home_visit);
                setCost3(res.data.payment.cost3);
                setNumber2(res.data.payment.number2 ? res.data.payment.number2 : '');
                setAdditionalPayment4(res.data.payment.additional_payment_4 ? res.data.payment.additional_payment_4 : '');
                setTotalPayment(res.data.payment.total_payment);
                setRemark(res.data.payment.remark ? res.data.payment.remark : '');
            }
          ...
          <div className="grid md:grid-cols-2 grid-cols-1 items-start gap-4 text-sm my-6">
              <div>
                  <form onSubmit={handleSubmit}>
                    <div className="grid md:grid-cols-2 gid-cols-1 gap-2.5 items-start">
                      ...
                      <div className="flex flex-col">
                          <label>Additonal Payment 4 (Optional)</label>
                          <TextInput
                              type="number"
                              step="any"
                              value={additionalPayment4}
                              onChange={(e) => setAdditionalPayment4(e.target.value)}
              ...
              <div>
                ...
                <div className="flex flex-col mb-3 relative">
                    <label>Search Patient by name and select to change (Optional)</label>
                    <TextInput
                        type="text"
                        value={searchPatient}
                                ...
                                <div className="flex flex-col gap-2.5">

                                    {
                                        patient && controlPatientSearch ? 
                                            patient
                                            .filter((pt) => pt.first_name.toLowerCase().includes(searchPatient.toLocaleLowerCase()) || pt.last_name.toLowerCase().includes(searchPatient.toLocaleLowerCase()))
                                            .map((result,i) => 
                                                <div className="flex justify-between items-center border-b border-amber-100 text-sm" key={i}>
                                                    <p>{result.first_name} {' '} {result.last_name}</p>
                                                    <button className="cursor-pointer bg-gray-600 px-2.5 rounded-md text-xs text-slate-200 py-[1px]"
...

                    {
                      patient && !selectedPatient && (
                        <table className="w-full mt-6">
                          <thead>
                              <tr>
                                <th colSpan={2} className="border border-gray-200 px-3 py-1.5 bg-blue-200">Selected Patient Information</th>
                              </tr>
                          </thead>
                          <tbody>
                              <tr>
                                  ...
                                  <td className="border border-gray-200 px-3 py-1.5">
                                      {payment.patient.title} {' '} {payment.patient.first_name} {payment.patient.last_name}
                                  </td>
                              </tr>
                              <tr>
                                  <td className="border border-gray-200 px-3 py-1.5">
                              ...
                              <tr>
                                  ...
                                  <td className="border border-gray-200 px-3 py-1.5">
                                      {payment.patient.street}{', '} {payment.patient.house_number} {', '} {payment.patient.city} {', '} {payment.patient.postal_code}
                                  </td>
                              </tr>
                              <tr>
                                  <td className="border border-gray-200 px-3 py-1.5">
                              ...
                              <tr>
                                  ...
                                  <td className="border border-gray-200 px-3 py-1.5">
                                    {payment.patient.health_insurance_company}
                                  </td>
                              </tr>
                              <tr>
                                  <td className="border border-gray-200 px-3 py-1.5">
                                  ...
                                  <td className="border border-gray-200 px-3 py-1.5">
                                      {payment.patient.payment_free == 1 ? 'Yes' : 'No'}
                                  </td>
                              </tr>
                              <tr>
                                  <td className="border border-gray-200 px-3 py-1.5">
                                  ...
                                  <td className="border border-gray-200 px-3 py-1.5">
                                      {payment.patient.treatment_in_6_month == 1 ? 'Yes' : 'No'}
                                  </td>
                              </tr>
                              <tr>
                                  <td className="border border-gray-200 px-3 py-1.5">
                                  ...
                                  <td className="border border-gray-200 px-3 py-1.5">
                                      {payment.patient.private_patient == 1 ? 'Yes' : 'No'}
                                  </td>
                              </tr>
                              <tr>
                                  <td className="border border-gray-200 px-3 py-1.5">
...

                {
                  selectedPatient && (
                    <div className="border border-gray-100 rounded-md p-2 my-3 text-sm bg-blue-200">
                        <p className="mb-2">New Selected Patient Information</p>
                        <table className="w-full">
                            <tbody>
                                <tr>
                                    <td className="border border-gray-100 px-3 py-2">Name</td>
                                    <td className="border border-gray-100 px-3 py-2">
                                        {selectedPatient.title} {' '}{selectedPatient.first_name} {' '} {selectedPatient.last_name}
                                    </td>
                                </tr>
                                <tr>
                                    <td className="border border-gray-100 px-3 py-2">Date of Birth</td>
                                    <td className="border border-gray-100 px-3 py-2">{selectedPatient.dob}</td>
                                </tr>
                                <tr>
                                    <td className="border border-gray-100 px-3 py-2">Address</td>
                                    <td className="border border-gray-100 px-3 py-2">
                                        {selectedPatient.street}{', '}{selectedPatient.house_number}{', '}{selectedPatient.city}{', '}{selectedPatient.postal_code}
                                    </td>
                                </tr>
                            </tbody>
                        </table>
